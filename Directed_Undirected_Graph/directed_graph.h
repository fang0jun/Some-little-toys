...
public:
	...
	void add_edge_fortree(const int&, const int&, const T&); //添加从第一个顶点到第二个顶点的加权边。
	...
	void get_edges(const int&); //显示由顶点引出的弧的列表；
	...
	directed_graph<T> out_tree(const int&); //返回使用边缘从给定顶点开始的图的生成树 
